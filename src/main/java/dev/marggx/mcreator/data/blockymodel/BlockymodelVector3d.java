...
public class BlockymodelVector3d extends Vector3d {
    ...
    public void round(int p) {
        if (p < 0) return;
        if (p == 0) {
            this.x = MathUtil.fastRound(this.x);
            this.y = MathUtil.fastRound(this.y);
            this.z = MathUtil.fastRound(this.z);
            return;
        }
        this.x = MathUtil.round(this.x, p);
        this.y = MathUtil.round(this.y, p);
        this.z = MathUtil.round(this.z, p);
    }
}
